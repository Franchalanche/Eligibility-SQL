/*
This script is given "As Is" with no warranties and plenty of caveats. 
Use at your own risk!
For more on data profiling, see Chapter 10 in "SQL Server 2012 Data Integration Recipes", 
Apress, 2012
*/-----------------------------------------------------------------------
...
USE WorkBench -- Your database here
GO

DECLARE @TABLE_SCHEMA NVARCHAR(128) = 'dbo'  -- Your schema here
DECLARE @TABLE_NAME NVARCHAR(128) = 'xxEligibility_All_RAW' -- Your table here
DECLARE @ColumnListIN NVARCHAR(max) = ''    -- Enter a comma-separated list of specific columns
...
DECLARE @DistinctValuesMinimum NUMERIC(16,2) = 4000 -- Minimum number of distinct values to suggest a reference 
                                         -- table and/or perform domain analysis
DECLARE @BoundaryPercent NUMERIC(5,2) = 0.57 -- Percent of records at upper/lower threshold to suggest
                                             -- a possible anomaly
DECLARE @NullBoundaryPercent NUMERIC(5,2) = 90.00 -- Percent of NULLs to suggest a possible anomaly
DECLARE @DataTypePercentage NUMERIC(16,2) = 2 -- Percentage variance allowed when suggesting another data type 
                                    -- for a column
-----------------------------------------------------------------------
-- Process variables
-----------------------------------------------------------------------
...
DECLARE @CHARACTER_MAXIMUM_LENGTH NUMERIC(16,2)
DECLARE @ROWCOUNT NUMERIC(16,2) = 0
DECLARE @ColumnList VARCHAR(max) = ' '
DECLARE @TableCheck NUMERIC(16,2)
DECLARE @ColumnCheck NUMERIC(16,2)
DECLARE @DataTypeVariance NUMERIC(16,2)
-----------------------------------------------------------------------
-- Start the process:
BEGIN
TRY
...
BEGIN
 ...
 DECLARE @CharPosition NUMERIC(16,2)
...
IF OBJECT_ID('tempdb..#ProfileData') IS NOT NULL
 ...
 (
  ...
  Distinct_Ct int,
  Null_Ct int,
  Nulls_Pct NUMERIC(9,4),
  ZeroLength_Ct int,
  ZeroLength_Pct NUMERIC(9,4),
  IsNumeric_Ct int NULL,
  IsDate_Ct int NULL,
  NoDateWithHourminuteSecond NUMERIC(16,2) NULL,
  NoDateWithSecond NUMERIC(16,2) NULL,
  AtLimit_Ct int NULL,
...
SET @ROWCOUNTPARAM = '@ROWCOUNTOUT NUMERIC(16,2) OUTPUT'
...
-----------------------------------------------------------------------
SET @SQLMETADATA = 'INSERT INTO #ProfileData (ColumnDataLength,COLUMN_NAME,TABLE_SCHEMA,TABLE_NAME,DataType,MaxDataLength,MinDataLength,AvgDataLength,MaxDate,MinDate,NoDateWithHourminuteSecond,NoDateWithSecond,IsNumeric_Ct,IsDate_Ct,Null_Ct,ZeroLength_Ct,Distinct_Ct)'
...
WHILE @@FETCH_STATUS = 0 
 BEGIN 
  ...
  , AVG(LEN(' + QUOTENAME(@COLUMN_NAME) + ')*1.0)
  ,NULL
  ,NULL 
  ,NULL 
  ,NULL 
  ...
  ,(SELECT COUNT (*) from ' + QUOTENAME(@TABLE_SCHEMA) + '.' + QUOTENAME(@TABLE_NAME) + ' WHERE TRY_PARSE(' + QUOTENAME(@COLUMN_NAME) + ' AS DATE) IS NOT NULL) '
...
UPDATE #ProfileData
 SET Nulls_Pct = (Null_Ct / @ROWCOUNT) * 100
    ,ZeroLength_Pct = (ZeroLength_Ct / @ROWCOUNT) * 100
-- Add any comments
...
UPDATE #ProfileData
 ...
 WHERE IsDate_Ct BETWEEN (@ROWCOUNT -@DataTypeVariance) AND (@ROWCOUNT + @DataTypeVariance)
 AND DataType IN ('varchar', 'nvarchar', 'char', 'nchar')
UPDATE #ProfileData
  SET DataTypeComments = 'Possibly could be one of the NUMERIC types. '
 WHERE IsNumeric_Ct BETWEEN (@ROWCOUNT -@DataTypeVariance) AND (@ROWCOUNT + @DataTypeVariance)
 AND DataType IN ('varchar', 'nvarchar', 'char', 'nchar')
UPDATE #ProfileData
...
UPDATE #ProfileData
 ...
 WHERE (Nulls_Pct = 100 OR ZeroLength_Pct = 100)
 AND IsFK = 0
...
UPDATE #ProfileData
 ...
 WHERE Nulls_Pct >= @NullBoundaryPercent
...
UPDATE #ProfileData
 ...
 WHERE Distinct_Ct < @DistinctValuesMinimum
...
SELECT
  P.COLUMN_NAME
 ,CASE
  WHEN P.Nulls_Pct >= T.Threshold THEN 'Could benefit from sparse columns. '
  ELSE ''
...
-----------------------------------------------------------------------
  IF OBJECT_ID('tempdb..#LimitTest') IS NOT NULL
    ...
    CREATE TABLE #LimitTest (COLUMN_NAME VARCHAR(128), AtLimit_Ct BIGINT);
    DECLARE @advancedtestSQL VARCHAR(MAX) = 'INSERT INTO #LimitTest (COLUMN_NAME, AtLimit_Ct)' + CHAR(13)
    ...
    UPDATE M
      SET M.AtLimit_Ct = T.AtLimit_Ct
         ,M.DataTypeComments = 
           CASE
             WHEN CAST(T.AtLimit_Ct AS NUMERIC(36,2)) / CAST(@ROWCOUNT AS NUMERIC(36,2)) >= @BoundaryPercent THEN ISNULL(M.DataTypeComments,'') + 'Large numbers of data elements at the max/minvalues. '
